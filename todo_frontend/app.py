...
def do_register(username: str, password: str) -> tuple[bool, str | None]:
    try:
...
def header_bar() -> str:
    _ = st.caption(f"Backend: {API_BASE}")
    left, right = st.columns([1, 1])
    with left:
        _ = st.segmented_control("Modus", options=["Login", "Registrieren"], key="auth_mode")
    with right:
        if st.session_state.get("logged_in"):
            if st.button("Logout", use_container_width=True):
                st.session_state["logged_in"] = False
                st.session_state.pop("user", None)
                _ = st.success("Abgemeldet.")
                st.rerun()
    _ = st.divider()
    ...
    with st.form("login_form"):
        ...
        if submitted:
            if not username or not password:
                _ = st.error("Bitte Username und Passwort eingeben.")
            else:
                ok, msg = do_login(username, password)
                if ok:
                    _ = st.success(f"Willkommen, {st.session_state['user']['name']}!")
                    st.rerun()
                else:
                    _ = st.error(msg or "Unbekannter Fehler beim Login.")
    return None

def register_view() -> None:
    _ = st.title("Registrieren")
    with st.form("register_form"):
        ...
        if submitted:
            if not username or not pw1 or not pw2:
                _ = st.error("Alle Felder ausfüllen.")
            elif pw1 != pw2:
                _ = st.error("Passwörter stimmen nicht überein.")
            elif len(pw1) < 6:
                _ = st.error("Passwort zu kurz (min. 6 Zeichen).")
            else:
                ok, msg = do_register(username, pw1)
                if ok:
                    _ = st.success(f"Account erstellt. Eingeloggt als {username}.")
                    st.rerun()
                else:
                    _ = st.error(msg or "Registrierung fehlgeschlagen.")
    return None

def authed_view() -> None:
    user = st.session_state.get("user")
    _ = st.title(f"Willkommen, {user['name']}")
    user_id = user["id"]

    _ = st.subheader("Neues Todo")
    task = st.text_input("Task")
    description = st.text_input("Beschreibung")
    state = st.selectbox("Status", ["OPEN", "DONE"])  # ggf. im Backend ergänzen
    ...
    if st.button("Todo erstellen", use_container_width=True):
        ...
        try:
            ...
            if r.status_code == 200:
                _ = st.success("Todo gespeichert.")
                _ = st.json(r.json())
            else:
                _ = st.error(f"Fehler beim Speichern ({r.status_code}): {r.text}")
        except Exception as e:
            _ = st.error(f"Fehler beim Speichern: {e}")

    _ = st.subheader("Todos")
    try:
        ...
        if r.status_code == 200:
            ...
            _ = st.error(f"Fehler beim Laden ({r.status_code}): {r.text}")
    except Exception as e:
        _ = st.error(f"Fehler beim Laden: {e}")
    return None
